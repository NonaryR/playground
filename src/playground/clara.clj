(ns playground.clara
  (:require [clara.rules :as rules :refer [defrule insert]]
...
(defrule get-current-temperature
  ...
  =>
  (rules/insert! (->CurrentTemperature (:value ?current-temp) ?location)))
...
(defn run-examples []
  ...
  (let [session
        (-> (rules/mk-session 'playground.clara)
    ...
    (-> session
        ...
        (rules/fire-rules) ;; Prints an alert and reduce speed messages for devices in room 1
        ...
        (rules/fire-rules))) ;; Prints increase speed messages for devices in room 1.
